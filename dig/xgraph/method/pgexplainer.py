...
class PGExplainer(nn.Module):
    ...
    def get_subgraph(self,
        ...
        if y is not None:
            y = y[subset]
        return x, edge_index, y, subset, kwargs, mapping
    ...
    def train_explanation_network(self, dataset, precompute_netx=False, large_dataset=False, batch_size=512):
        ...
        if self.explain_graph:
            ...
            for epoch in range(self.epochs):
                ...
                optimizer.step()
                duration += time.perf_counter() - tic
                print(f'Epoch: {epoch} | Loss: {loss.item()/len(explain_node_index_list)}')
    ...
    def forward(self,
        ...
        if self.explain_graph:
            ...
            # masked value
            x, edge_index, _, subset, _, _ = self.get_subgraph(node_idx, x, edge_index)
    ...
    def visualization(self, data: Data, edge_mask: Tensor, top_k: int, plot_utils: PlotUtils,
        ...
        if self.explain_graph:
            ...
            x, edge_index, y, subset, kwargs, _ = \
